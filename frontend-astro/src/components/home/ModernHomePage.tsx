...
import {ArrowRight, Clock, TrendingUp, Eye, User, Calendar, ChevronRight, Menu, X} from 'lucide-react';
        ...
        <section className="max-w-6xl mx-auto px-4 sm:px-6 py-12 sm:py-16">
          <div className="grid lg:grid-cols-5 gap-8 items-center">
            ...
            <div className="lg:col-span-2">
              ...
              <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white mt-2 mb-4 leading-tight">
                <a href={`/articles/${f.slug}`} className="hover:text-blue-600 transition-colors">{f.title}</a>
              </h2>
              ...
              <div className="flex items-center gap-4 text-xs text-gray-400">
                ...
                <a href={`/articles/${f.slug}`} className="ml-auto text-blue-600 hover:text-blue-700 font-medium flex items-center gap-1">阅读全文 <ChevronRight className="w-3.5 h-3.5"/></a>
              </div>
            </div>
          </div>
          {/* Sub featured */}
          {fSub.length > 0 && (
            <div className="grid sm:grid-cols-3 gap-4 mt-6">
              {fSub.map(a => (
                <a key={a.id} href={`/articles/${a.slug}`} className="group p-4 rounded-xl border border-gray-100 dark:border-gray-800 hover:border-gray-200 dark:hover:border-gray-700 transition-all">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-12 sm:py-16">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {recent.slice(0, 4).map(a => (
              <a key={a.id} href={`/articles/${a.slug}`} className="group bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 overflow-hidden hover:border-gray-200 dark:hover:border-gray-700 transition-all hover:shadow-sm">
        ...
        <div className="lg:col-span-2">
          ...
          <div className="space-y-3">
            {popular.slice(0, 5).map((a, i) => (
              <a key={a.id} href={`/articles/${a.slug}`} className="flex items-center gap-4 p-3 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors group">

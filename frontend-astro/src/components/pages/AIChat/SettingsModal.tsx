...
import {X, Eye, EyeOff, Check, Sparkles, Settings} from 'lucide-react';
...
export default function SettingsModal({config, onChange, onClose, show}: SettingsModalProps) {
  ...
  const [tab, setTab] = useState<'manual' | 'saved'>('manual');
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4" onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-gray-800 flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400">配置 LLM 连接</p>
            </div>
          </div>
        ...
        {/* Tabs */}
        <div className="flex border-b border-gray-100 dark:border-gray-800 px-5 gap-4">
          <button onClick={() => setTab('manual')}
            className={`pb-2.5 text-xs font-medium border-b-2 transition-all ${tab === 'manual' ? 'border-violet-500 text-violet-600 dark:text-violet-400' : 'border-transparent text-gray-400 hover:text-gray-600'}`}>
            手动配置
          </button>
          <button onClick={() => setTab('saved')}
            className={`pb-2.5 text-xs font-medium border-b-2 transition-all flex items-center gap-1.5 ${tab === 'saved' ? 'border-violet-500 text-violet-600 dark:text-violet-400' : 'border-transparent text-gray-400 hover:text-gray-600'}`}>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          {tab === 'saved' ? (
            <AIConfigManager onConfigSelect={(saved) => {
              onChange({endpoint: saved.api_url, apiKey: saved.api_key || '', model: saved.model});
              setTestOk(true);
            }} />
          ) : (
            <div className="space-y-5">
              ...
              <div>
                ...
                <input value={config.endpoint} onChange={e => { onChange({endpoint: e.target.value}); setTestOk(false); }}
              ...
              <div>
                ...
                <div className="relative">
                  <input type={showKey ? 'text' : 'password'} value={config.apiKey}
                    onChange={e => { onChange({apiKey: e.target.value}); setTestOk(false); }}
                    placeholder="sk-..."
              ...
              <div>
                ...
                <input value={config.model} onChange={e => { onChange({model: e.target.value}); setTestOk(false); }}
                  placeholder="gpt-4o"
                  className="w-full px-3.5 py-2.5 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-sm dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-violet-500/40 focus:border-violet-400 transition-all" />
              </div>
              ...
              {config.endpoint && config.model && (
                <button onClick={() => setTestOk(!testOk)}
        ...
        <div className="flex items-center justify-between px-5 py-3.5 border-t border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-800/30 flex-shrink-0">
          <span className="text-[10px] text-gray-400 dark:text-gray-500">
            {tab === 'saved' ? '配置保存在服务端，加密存储' : '手动配置仅保存在浏览器本地'}
          </span>

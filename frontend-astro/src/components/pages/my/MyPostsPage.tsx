...
function MyPostsInner() {
  ...
  const {data, isLoading} = useQuery({
    ...
    queryFn: async () => {
      ...
      if (res.data.data) return {
        articles: (res.data.data as any).articles || [],
  ...
  return (
      <div
          className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 dark:from-gray-950 dark:to-gray-900 pt-20">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 pb-12">
          ...
          <div className="space-y-2">
            {isLoading ? (
                <div className="space-y-2">{[1, 2, 3, 4].map(i => <div key={i}
                                                                       className="h-20 bg-white/50 dark:bg-gray-800/50 rounded-xl animate-pulse"/>)}</div>
            ) : articles.length === 0 ? (
                <div
                    ...
                    <div key={a.id}
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                                        <span
                                            className={`px-2 py-0.5 text-xs rounded-full font-medium ${isPublished(a.status) ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>{statusLabel(a.status)}</span>
                          {a.hidden && <Lock className="w-3 h-3 text-gray-400"/>}
                        </div>
                        <div className="flex items-center gap-2">
                          <a href={`/view?slug=${a.slug}`}
              ...
              <div className="flex justify-center gap-1.5 mt-6">
                {Array.from({length: pages}, (_, i) => i + 1).map(p => (
                    <button key={p} onClick={() => setPage(p)}
                            className={`px-3 py-1.5 rounded-lg text-sm ${p === page ? 'bg-blue-600 text-white' : 'border border-gray-200 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-800'}`}>{p}</button>
                ))}
              </div>
          )}
        </div>

    ...
    <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 overflow-hidden">
      ...
      <div
        ...
        <div className="flex items-center gap-3">
          ...
          >
            {editing ? (
              <Edit3 className="w-4.5 h-4.5 text-amber-600 dark:text-amber-400"/>
            ) : (
      ...
      <div
        ...
        <p className="text-xs text-gray-400">{editing ? '修改后将立即生效' : '创建后可在列表中编辑'}</p>
...
function CategoriesInner() {
  ...
  const batchDelMut = useMutation({
    mutationFn: (ids: number[]) =>
      Promise.all(ids.map((id) => apiClient.delete(CATEGORIES.DELETE(id)))),
    onSuccess: () => {
      qc.invalidateQueries({queryKey: ['admin-categories']});
  ...
  const stats = useMemo(() => {
    ...
    const totalArticles = cats.reduce(
      (sum: number, c: any) => sum + (c.articles_count || c.article_count || 0),
      0
    );
    const sorted = [...cats].sort(
      (a: any, b: any) =>
        (b.articles_count || b.article_count || 0) -
        (a.articles_count || a.article_count || 0)
    );
    return {
      total,
      totalArticles,
            ...
            {batchMode ? (
              <>
                ...
                <button
                  ...
                  className="px-2 py-1.5 text-xs text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg transition-colors"
                >
                  取消
                </button>
                <button
                  onClick={() => {
                    if (selectedIds.size > 0 && confirm('确定删除选中的分类？'))
                      batchDelMut.mutate(Array.from(selectedIds));
                  }}
                  className="px-2 py-1.5 text-xs text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors"
                >
                  删除选中
                </button>
              </>
      ...
      {isLoading ? (
        ...
        <div
          ...
          <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
            <SortableContext
              items={filteredCats.map((c: any) => c.id)}
          ...
          {/* 底部 */}
          <div className="px-5 py-3 border-t border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-800/10">
            <p className="text-xs text-gray-400">
              共 {filteredCats.length} 个分类
...
export default function AdminCategories() {
  return (
    <AuthGuard>
      <QueryProvider>
        <PermissionGuard capability="category:view">
          <CategoriesInner/>
        </PermissionGuard>
      </QueryProvider>
    </AuthGuard>

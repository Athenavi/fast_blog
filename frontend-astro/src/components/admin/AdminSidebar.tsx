...
/** @deprecated 浣跨敤 navConfig 鏇夸唬锛屼繚鐣欑敤浜庡悜鍚庡吋瀹?*/
      ...
      <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
        {navConfig.map((item, i) => {
          if (item.sepKey) return <div key={`sep-${i}`}
                                       className={`pt-3 pb-1 text-xs font-semibold text-gray-400 uppercase tracking-wider ${collapsed ? 'text-center text-[10px]' : 'px-3'}`}>{collapsed ? '路路' : t(item.sepKey)}</div>;

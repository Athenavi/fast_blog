  ...
  return (
    <aside
      ...
      <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
        {nav.map((item, i) => {
          if ('sep' in item) return <div key={`sep-${i}`}
                                         className={`pt-3 pb-1 text-xs font-semibold text-gray-400 uppercase tracking-wider ${collapsed ? 'text-center text-[10px]' : 'px-3'}`}>{collapsed ? '··' : item.sep}</div>;
    ...
    <>
      ...
      <aside
        ...
        <nav className="p-3 space-y-1 overflow-y-auto"
          ...
          {nav.map((item, i) => {
            if ('sep' in item) return <div key={`sep-${i}`}
                                           className="pt-3 pb-1 px-3 text-xs font-semibold text-gray-400 uppercase tracking-wider">{item.sep}</div>;
            const Icon = item.icon!;
            return (

...
if (-not (Test-Path "..\frontend\web")) {
    Write-Host "❌ 错误: frontend/web 目录不存在" -ForegroundColor Red
    exit 1
}

Write-Host "✅ frontend/web 目录存在" -ForegroundColor Green

# 检查 Astro 项目是否已构建
if (-not (Test-Path "..\frontend\web\.output\public")) {
    Write-Host "⚠️  警告: frontend/web/.output/public 目录不存在，需要先构建" -ForegroundColor Yellow
    Write-Host "   运行: cd ..\frontend\web; npm run generate" -ForegroundColor Yellow
} else {
    Write-Host "✅ frontend/web/.output/public 目录存在" -ForegroundColor Green
    
    # 检查 dist 目录是否有内容
    $distFiles = Get-ChildItem "..\frontend\web\.output\public" -Recurse -File
...
if ($configContent -match '"webDir":\s*"../frontend/web/.output/public"') {
    Write-Host "✅ webDir 配置正确: ../frontend/web/.output/public" -ForegroundColor Green
} else {
...
if ($packageContent -match '"build:web":\s*"cd ../frontend/web && npm run generate"') {
...
Write-Host "  1. 构建 Astro 前端: cd ..\frontend\web; npm run generate" -ForegroundColor White

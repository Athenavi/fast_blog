...
export default function CustomBlockManager() {
    ...
    const loadData = async () => {
        try {
            ...
            // 加载所有块
            const blocksResponse = await fetch('/api/v1/custom-blocks/blocks');
            ...
            // 加载插件列表
            const pluginsResponse = await fetch('/api/v1/custom-blocks/plugins');
    ...
    const togglePlugin = async (pluginName: string, isActive: boolean) => {
        try {
            ...
            const response = await fetch(`/api/v1/custom-blocks/plugins/${pluginName}/${endpoint}`, {
                method: 'POST'
            });
        ...
        <div className="space-y-6">
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab}>
                <TabsList className="grid w-full grid-cols-2">
                    <TabsTrigger value="blocks">块类型 ({blocks.length})</TabsTrigger>
                            ...
                            <CardContent>
                                <div className="flex items-center gap-4 text-sm">
                                    <div className="flex items-center gap-1">
                                        <span className="text-gray-600 dark:text-gray-400">块数量:</span>
            ...
            <Card className="bg-blue-50 dark:bg-blue-950 border-blue-200 dark:border-blue-800">
                <CardContent className="pt-6">
                    ...
                    <ol className="list-decimal list-inside space-y-2 text-sm text-blue-800 dark:text-blue-200">
                        <li>继承 <code
                            className="bg-white dark:bg-gray-800 px-2 py-1 rounded">CustomBlockPlugin</code> 类
                        </li>
                        <li>实现 <code
                            className="bg-white dark:bg-gray-800 px-2 py-1 rounded">register_blocks()</code> 方法注册块类型
                        </li>
                        <li>可选：实现 <code
                            className="bg-white dark:bg-gray-800 px-2 py-1 rounded">render_{'{block_name}'}</code> 方法自定义渲染
                        </li>
                        <li>将插件文件放在 <code
                            className="bg-white dark:bg-gray-800 px-2 py-1 rounded">shared/services/</code> 目录
                        </li>
                        <li>在 API 中导入并加载插件</li>
                    </ol>
                </CardContent>
            </Card>
        </div>

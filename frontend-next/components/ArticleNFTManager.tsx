...
export default function ArticleNFTManager({articleId, articleTitle}: ArticleNFTManagerProps) {
    ...
    const loadNFTInfo = async () => {
        try {
            const response = await fetch(`/api/v1/nft/${articleId}`);
            if (response.ok) {
    ...
    const connectWallet = async () => {
        // 在实际实现中，这里应该使用 web3.js 或 ethers.js 连接真实钱包
        // 这里仅作为演示
        ...
        if (typeof window !== 'undefined' && (window as any).ethereum) {
            ...
            // 如果没有安装 MetaMask，提示用户
            setError('请安装 MetaMask 或其他 Web3 钱包扩展');
        }
    };
            ...
            <Card
                ...
                <CardHeader>
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            ...
                            <CardTitle className="text-lg">NFT 已铸造</CardTitle>
                        </div>
                ...
                <CardContent className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                        ...
                        <div>
                            <Label className="text-xs text-gray-500">铸造时间</Label>
        ...
        <>
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>
                        将文章 "{articleTitle}" 铸造为 NFT，获得区块链上的所有权证明
                    </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                    {error && (
                    ...
                    <Dialog open={showMintDialog} onOpenChange={setShowMintDialog}>
                        ...
                        <DialogContent>
                            <DialogHeader>
                                <DialogTitle>确认铸造 NFT</DialogTitle>
                                <DialogDescription>
                                    这将把文章 "{articleTitle}" 铸造为 NFT
                                </DialogDescription>
                            </DialogHeader>

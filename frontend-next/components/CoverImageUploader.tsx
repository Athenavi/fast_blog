...
import {apiClient, MediaFile, MediaService} from '@/lib/api';
...
const CoverImageUploader: React.FC<CoverImageUploaderProps> = ({
    ...
    const handleMediaSelect = async (media: MediaFile) => {
        try {
            // 优先使用媒体文件的 url 字段，如果没有则根据 hash 生成 URL
            let coverUrl: string;
            ...
            if (media.url) {
                ...
                const {getConfig} = await import('@/lib/config');
                const config = getConfig();
                // API_BASE_URL 已经是基础 URL（如 http://localhost:9421），直接拼接
                coverUrl = `${config.API_BASE_URL}${media.url}`;
            } else {
                throw new Error('媒体文件缺少 url 或 hash 字段');
            }

            onChange(coverUrl);
            setIsOpen(false);
            setValidationStatus('valid');
            setValidationMessage('已选择封面图片');
            onValidationChange?.(true);
        } catch (error) {

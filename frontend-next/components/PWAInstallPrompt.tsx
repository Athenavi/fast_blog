...
export default function PWAInstallPrompt({showPrompt = true}: PWAInstallPromptProps) {
    ...
    const [showOfflineMessage, setShowOfflineMessage] = useState(false); // 控制离线消息显示
    ...
    useEffect(() => {
        ...
        const handleOnline = () => {
            setIsOffline(false);
            setShowOfflineMessage(false); // 网络恢复后隐藏离线消息
            console.log('[PWA] Network online');
        };
        const handleOffline = () => {
            setIsOffline(true);
            setShowOfflineMessage(true); // 网络断开时显示离线消息
        ...
        // 初始化时设置正确的网络状态 - 添加延迟确保准确检测
        setTimeout(() => {
            const isCurrentlyOffline = !navigator.onLine;
            setIsOffline(isCurrentlyOffline);
            setShowOfflineMessage(isCurrentlyOffline); // 初始状态如果是离线则显示消息
            console.log('[PWA] Initial network status in PWAInstallPrompt:', navigator.onLine ? 'online' : 'offline');
        }, 150);
    ...
    if (!shouldShow && !showOfflineMessage && !updateAvailable) {
        return null;
    }

...
const OptimizedMarkdownEditor: React.FC<OptimizedMarkdownEditorProps> = ({ 
  ...
  // 处理媒体文件选择（支持单个或多个文件）
  const handleMediaSelect = useCallback(async (media: MediaFile | MediaFile[]) => {
    if (onInsertMedia) {
      onInsertMedia(media);
    } else {
      // 处理单个文件
      ...
      // 为每个文件生成对应的 Markdown 格式
        const mediaMarkdowns = mediaFiles.map((singleMedia) => {
            const mediaUrl = getMediaUrlSync(String(singleMedia.id));

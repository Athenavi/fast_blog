...
export default function TemplateHierarchyPage() {
  ...
  const loadHierarchy = async () => {
    try {
        const response = await fetch('/api/v2/template-hierarchy/hierarchy');
      if (response.ok) {
  ...
  const loadTemplates = async () => {
    try {
      const token = getAccessTokenFromCookie();
        const response = await fetch(`/api/v2/template-hierarchy/templates/${theme}`, {
        headers: {
  ...
  const handleResolveArticle = async () => {
    ...
    try {
      setLoading(true);
      const token = getAccessTokenFromCookie();
        const response = await fetch('/api/v2/template-hierarchy/resolve/article', {
        method: 'POST',
        headers: {

...
export default function VideoTranscodePage() {
    ...
    const loadData = async () => {
        try {
            ...
            const [configRes, tasksRes] = await Promise.all([
                fetch('/api/v2/media/video-transcode/config', {
                    headers: {'Authorization': `Bearer ${token}`},
                }),
                fetch('/api/v2/media/video-transcode/tasks', {
                    headers: {'Authorization': `Bearer ${token}`},
                }),
            ]);
    ...
    const saveConfig = async () => {
        try {
            setSaving(true);
            const token = getAccessToken();
            const response = await fetch('/api/v2/media/video-transcode/config', {
                method: 'POST',
                headers: {
    ...
    const startTranscode = async (videoId: number) => {
        try {
            const token = getAccessToken();
            const response = await fetch('/api/v2/media/video-transcode/start', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`,
    ...
    const deleteTask = async (taskId: number) => {
        ...
        try {
            const token = getAccessToken();
            const response = await fetch(`/api/v2/media/video-transcode/tasks/${taskId}`, {
                method: 'DELETE',
                headers: {'Authorization': `Bearer ${token}`},
            });

...
export default function MigrationsPage() {
  ...
  const loadStatus = async () => {
    try {
      const token = getAccessTokenFromCookie();
        const response = await fetch('/api/v2/migrations/status', {
        headers: {
  ...
  const handleApplyMigrations = async () => {
    ...
    try {
      setLoading(true);
      const token = getAccessTokenFromCookie();
        const response = await fetch('/api/v2/migrations/apply', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
  ...
  const handleCreateMigration = async () => {
    ...
    try {
      const token = getAccessTokenFromCookie();
        const response = await fetch('/api/v2/migrations/create', {
        method: 'POST',
        headers: {
  ...
  const handleRollback = async () => {
    ...
    try {
      const token = getAccessTokenFromCookie();
        const response = await fetch('/api/v2/migrations/rollback', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,

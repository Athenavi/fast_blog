...
export function ArticleRevisionsPage() {
    ...
    const fetchRevisions = async () => {
        ...
        try {
            setLoading(true);
            const response = await fetch(`/api/v2/articles/${articleId}/revisions`);
    ...
    const performComparison = async (rev1Id: number, rev2Id: number) => {
        try {
            const response = await fetch(
                `/api/v2/articles/revisions/compare?revision1_id=${rev1Id}&revision2_id=${rev2Id}`
            );
            const data = await response.json();
    ...
    const handleConfirmRollback = async () => {
        ...
        try {
            const response = await fetch(
                `/api/v2/articles/${articleId}/revisions/${revisionToRollback.id}/rollback`,
                {method: 'POST'}
            );
            const data = await response.json();

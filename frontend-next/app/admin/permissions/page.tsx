...
const PermissionManagement = () => {
  const [activeTab, setActiveTab] = useState('roles');
    ...
    // 角色状态  const [roles, setRoles] = useState<Role[]>([]);
  ...
  const [roleForm, setRoleForm] = useState({
    ...
    // 权限状态  const [permissions, setPermissions] = useState<Permission[]>([]);
  const [permissionSearch, setPermissionSearch] = useState('');

    // 用户状态  const [users, setUsers] = useState<User[]>([]);
  ...
  const loadUsers = async () => {
    ...
    // 打开角色编辑对话框  const handleEditRole = (role: Role) => {
...
// 打开新建角色对话框  const handleCreateRole = () => {
...
// 全选取消全选  const toggleAllPermissions = () => {
...
// 按分类分组权限  const groupedPermissions = permissions.reduce((acc, perm) => {

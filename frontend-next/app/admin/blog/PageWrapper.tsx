    ...
    try {
      ...
      const response = await ArticleManagementService.getArticles(params);
        ...
        // 确保response.data有正确的结构
            ...
            if (typeof responseData === 'object' && 'articles' in responseData) {
                setArticles(responseData.articles || []);
                setTotalPages(responseData.pagination?.pages || responseData.pagination?.total_pages || 1);
        } else {
          // 如果没有按预期格式返回，则直接使用数据
  ...
  const loadAuthors = async () => {
    try {
      const response = await UserManagementService.getUsers();
        ...
        // 确保response.data有正确的结构
            const responseData = response.data as any;
            if (typeof responseData === 'object' && 'users' in responseData) {
                setAuthors(responseData.users || []);
        } else {
          // 如果没有按预期格式返回，则直接使用数据
  ...
  return (
    <div className="space-y-6">
      ...
      <KeyboardShortcutsHelp shortcuts={[
        { keys: 'Ctrl + K', description: '搜索文章' },
        { keys: 'N', description: '新建文章' },

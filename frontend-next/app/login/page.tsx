...
const LoginPage = () => {
  ...
  // 检测是否在 Capacitor 环境中，如果是则跳转到移动端登录页面
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const checkAuthAndRedirect = async () => {
      ...
      try {
        // 简单检查 cookie 中的令牌是否存在,而不通过 API 客户端
        ...
        // 如果 access_token 不存在但 refresh_token 存在,尝试刷新 token
        if (!accessToken && refreshToken) {
          ...
          try {
            ...
            if (refreshResponse.success && refreshResponse.data) {
              const refreshData = refreshResponse.data as { access_token?: string; refresh_token?: string };
                
              if (refreshData.access_token) {
                // 保存新的 access_token
                const expirationDate = new Date();
                expirationDate.setTime(expirationDate.getTime() + (60 * 60 * 1000)); // 1 小时后过期
                document.cookie = `access_token=${refreshData.access_token}; expires=${expirationDate.toUTCString()}; path=/; SameSite=Lax;`;
                  
                // 如果有新的 refresh_token,也更新
                if (refreshData.refresh_token) {
                  const refreshExpirationDate = new Date();
                  refreshExpirationDate.setTime(refreshExpirationDate.getTime() + (7 * 24 * 60 * 60 * 1000)); // 7 天后过期
                  document.cookie = `refresh_token=${refreshData.refresh_token}; expires=${refreshExpirationDate.toUTCString()}; path=/; SameSite=Lax;`;
                }
            ...
            console.log('Token refresh failed:', (refreshError as Error)?.message || String(refreshError));
            // 刷新失败,清除无效的 refresh_token
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        // 登录失败，显示错误消息
        setErrorMessage(response.error || response.message || '登录失败，请检查用户名和密码');
      }
    } catch (error: unknown) {
  ...
  const handle2FASubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await apiClient.post('/security/2fa/verify-login', {
        user_id: userId,
        token: twoFACode
      });

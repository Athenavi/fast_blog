...
const RegisterPage = () => {
  ...
  // 检测是否在 Capacitor 环境中，如果是则跳转到移动端注册页面
  useEffect(() => {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (registerForm.password !== registerForm.confirmPassword) {
      ...
      console.error('请同意服务条款和隐私政策');
          alert('请在第 4 步"偏好设置和条款"中勾选"我同意服务条款和隐私政策"');
      return;
    }
      ...
      try {
          // 调用后端注册 API - 使用表单格式（兼容 FastAPI/Django）
          const response = await apiClient.postForm<any>('/management/auth/register', {
              ...
              if (response.data && (response.data as any).access_token) {
                  if (typeof window !== 'undefined') {
                      ...
                      if ((response.data as any).refresh_token) {
                          ...
                          document.cookie = `refresh_token=${(response.data as any).refresh_token}; expires=${refreshExpirationDate.toUTCString()}; path=/; SameSite=Strict;`;
                      }
                  }
                  ...
                  router.refresh(); // 刷新路由以更新 UI
              }
      } else {
        console.error('注册失败:', response?.error || '未知错误');
        // 显示错误消息
              alert(response?.error || response?.message || '注册失败，请稍后重试');
      }
    } catch (error) {
      console.error('注册过程中发生错误:', error);
          const errorObj = error as any;
          alert(errorObj.message || '网络错误或服务器无响应，请稍后重试');
    } finally {
    ...
    if (username.length < 3) {
        setUsernameFeedback({message: '用户名至少需要 3 个字符', type: 'error'});
      return false;
    }

        // 调用 API 检查用户名是否可用
        try {
            const response = await apiClient.get<any>(`/check-username?username=${username}`);
            if ((response as any).exists) {
                setUsernameFeedback({message: '用户名已被使用', type: 'error'});
                return false;
            } else {
            ...
            setUsernameFeedback({message: '检查用户名失败', type: 'error'});
            return false;
        }
  };
    ...
    if (!emailRegex.test(email)) {
      ...
      return false;
        ...
        // 调用 API 检查邮箱是否可用
        try {
            const response = await apiClient.get<any>(`/check-email?email=${email}`);
            if ((response as any).exists) {
                setEmailFeedback({message: '邮箱已被注册', type: 'error'});
                return false;
            } else {
            ...
            setEmailFeedback({message: '检查邮箱失败', type: 'error'});
            return false;
        }
  };

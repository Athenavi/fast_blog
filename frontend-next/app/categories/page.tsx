...
const CategoryListPage = () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        setPagination({
          ...
          has_next: (response.data as any).pagination?.has_next || page < Math.ceil(((response.data as any).pagination?.total || 0) / 12),
          current_page: (response.data as any).pagination?.current_page || page,
          total_pages: (response.data as any).pagination?.total_pages || Math.ceil(((response.data as any).pagination?.total || 0) / 12),
  ...
  const goToCategory = (categoryName: string) => {
    router.push(`/category/detail?name=${encodeURIComponent(categoryName)}`);
  };

#!/usr/bin/env python3
"""T5-11 批次 10：admin_menus 补插 Monitoring（监控中心）一行。

菜单 code 必须与前端 ``frontend/web/src/utils/menus.ts`` 的 ``name`` 一一对应
（``Monitoring`` → ``/system/monitoring``）。

父菜单按 **code** 查找（``System``），不硬编码 id。

用法::

    python -m scripts.seed_batch10_menus            # dry-run：只报告现状
    python -m scripts.seed_batch10_menus --apply    # 幂等写库（已存在的 code 跳过）
"""
...
NEW_MENUS: list[dict] = [
    {
        ...
        "parent_code": "System",
        "permission_code": "module_system:monitor:view",
        "sort_order": 30,
    },
]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="补插 T5-11 批次 10 的后台菜单行")

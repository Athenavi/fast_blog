"""
路由代码生成器 - 根据 routes.yaml 声明文件自动生成 Django Ninja 和 FastAPI 的路由代码
使用方法:
...
class RouteGenerator:
    ...
    def generate_all(self):
        ...
        # 1. 生成 Django ORM Mixins（已在 v0.2 淘汰）
    ...
    def generate_single_model(self, model_names: List[str]):
        ...
        for model_name in model_names:
            ...
            try:
                ...
                # 从 properties 生成字段，传入模型名称和全量模型配置（用于外键解析）
                properties = model_def.get('properties', {})
                fields = self._convert_properties_to_fields(
                    properties,
                    model_name=model_name,
                ...
                for field in fields.values():
                    if field.get('default') == 'datetime.utcnow':
                        has_datetime_default = True
                    if field.get('type') in ('Numeric', 'decimal'):
                        has_numeric = True
                ...
                rendered = template.render(
                    ...
                    is_unlogged=model_def.get('unlogged', False),
                    custom_methods=custom_methods,
                    ns=type('NS', (), {'has_uuid_pk': has_uuid_pk, 'has_datetime_default': has_datetime_default})(),
                    all_models=self.models,
                )
    ...
    def _check_list_fields(self, model_def: Dict) -> bool:
        ...
        for field_def in model_def.get('fields', {}).values():
            if field_def.get('type') in ('array', 'list'):
                return True
        return False
...
"""
    ...
    def sync_to_django(self, app_name: str = None):
        ...
        # 2. 如果指定了 app，生成该 app 的 models.py
        if app_name:

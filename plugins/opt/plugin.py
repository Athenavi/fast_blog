...
class OptimizationPlugin(BasePlugin):
    ...
    def convert_to_cdn_url(self, original_url: str) -> str:
        """将原始URL转换为CDN URL"""
        if not self.settings.get('enable_cdn') or not self.settings.get('cdn_base_url'):
            return original_url

        # 检查是否是静态资源或媒体文件
        if original_url.startswith(('/static', '/media')):
            cdn_url = f"{self.settings['cdn_base_url']}{original_url}"
            return cdn_url

        return original_url

    def convert_html_images_to_cdn(self, html_content: str) -> str:
        """将HTML中的图片URL转换为CDN URL"""
        if not self.settings.get('enable_cdn'):
            return html_content
        ...
        def replace_src(match):
            ...
            if not src_url.startswith(('http://', 'https://')):
                cdn_url = self.convert_to_cdn_url(src_url)
                img_tag = img_tag.replace(src_url, cdn_url)

            return img_tag

        pattern = r'<img[^>]+src=["\']([^"\']+)["\'][^>]*>'
        converted_html = re.sub(pattern, replace_src, html_content)
    ...
    def get_dashboard_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'cdn_enabled': self.settings.get('enable_cdn', False),

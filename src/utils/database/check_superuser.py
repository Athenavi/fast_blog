...
def create_default_superuser(session: Session) -> Optional[User]:
    ...
    Returns:
        User: 创建的超级管理员用户对象，如果创建失败则返回 None
    """
    print("\n正在自动创建默认超级管理员用户...")
    ...
    import secrets
    import string
    default_password = ''.join(secrets.choice(string.ascii_letters + string.digits + string.punctuation) for _ in range(16))
    ...
    # 生成随机强密码
    import secrets
    default_password = secrets.token_urlsafe(16)  # 生成 16 字节的随机密码
    
    print("=" * 60)
    print("⚠️  已自动生成随机强密码，请妥善保存：")
    print(f"用户名：{default_username}")
    print(f"密  码：{default_password}")
    print("⚠️  首次登录后请立即修改密码！")
    print("此密码已记录到日志文件，请在安全位置备份。")
    print("=" * 60)
    
    # 将密码记录到日志文件（仅一次）
    try:
        log_file = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'logs', 'admin_password.log')
        with open(log_file, 'a', encoding='utf-8') as f:
            from datetime import datetime
            f.write(f"[{datetime.now().isoformat()}] 默认管理员密码：{default_password}\n")
    except Exception as e:
        print(f"警告：无法将密码记录到日志文件：{e}")

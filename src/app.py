...
- 🔌 插件系统基础框架
    ...
    try:
        ...
        # 全文搜索引擎 API (Meilisearch)
        try:
            from src.api.v1.fulltext_search import router as fulltext_search_router
            app.include_router(fulltext_search_router, prefix='/api/v1', tags=['fulltext-search'])
            print(f"{worker_info} [OK] FullText Search API 已加载")
        except ImportError as e:
            print(f"Warning: FullText Search API could not be loaded: {e}")

        # 外部资源转存 API
        try:
            from src.api.v1.resource_transfer import router as resource_transfer_router
            app.include_router(resource_transfer_router, prefix='/api/v1', tags=['resource-transfer'])
            print(f"{worker_info} [OK] Resource Transfer API 已加载")
        except ImportError as e:
            print(f"Warning: Resource Transfer API could not be loaded: {e}")

...
async def api_user_avatar(
    ...
    avatar_url = await get_avatar(domain=domain, user_identifier=user_identifier,
                                  identifier_type=identifier_type, db=db)
    if avatar_url:
        return avatar_url
    else:
        # 默认头像服务器地址
...
async def _generate_unique_tags():
    ...
    # 获取所有文章的标签字符串
    tags_results = db_session.query(Article.tags_list).all()  # 修复：使用 tags_list 而不是 tags
...
async def confirm_email_change(
    ...
    return await confirm_email_back(current_user_obj.id, cache, token, db)
...
async def update_article_status(
    ...
    article_query = select(Article).where(
        ...
        Article.user == current_user_obj.id
    )

...
from src.api.v1.user_utils.user_entities import check_user_conflict, change_username, db_save_bio, \
    ...
    try:
        ...
        for field in ['username', 'email', 'bio', 'profile_private', 'locale']:
            if field in data:
                if field == 'username':
                    # 检查用户名修改限制
                    limit_username_lock = cache.get(f'limit_username_lock_{current_user.id}')
                    ...
                    await change_username(current_user.id, data[field], db)
                    cache.set(f'limit_username_lock_{current_user.id}', True, ex=604800)
                elif field == 'email':
                    # 检查邮箱冲突
                    if check_user_conflict('email', data[field], db):
                        return ApiResponse(success=False, error='Email already exists')
                    # 请求邮箱变更
                    await request_email_change(current_user.id, cache, app_config.domain, data[field])
                elif field == 'bio':
                    await db_save_bio(current_user.id, data[field], db)
                elif field == 'locale':
                    if not valid_language_codes(data[field]):
                        return ApiResponse(success=False, error='Invalid locale')
                    current_user.locale = data[field]
                else:
        ...
        if current_user.profile_picture:
            ...
            safe_filename = safe_filename.replace('"', '').replace("'", "").strip()
            if safe_filename:
                base_url = str(request.url).split('/me')[0].rsplit('/', 1)[0]
                avatar_url = f"{base_url}/static/avatar/{safe_filename}.webp"

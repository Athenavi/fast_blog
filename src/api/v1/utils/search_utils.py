...
from src.models import Article, ArticleContent, SearchHistory
from src.utils.database.main import get_db
...
def get_user_search_history(user_id):
    with next(get_async_db()) as session:
        history_keywords = session.query(SearchHistory.keyword).filter(SearchHistory.user_id == user_id).order_by(

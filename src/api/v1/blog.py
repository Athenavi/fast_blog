...
async def get_article_by_slug_api(
    ...
    """
    try:
        print(f'\n🔍 [DEBUG] get_article_by_slug_api called')
        print(f'   - slug: {slug}')
        print(f'   - current_user: {current_user}')
        if current_user:
            print(f'   - current_user.id: {current_user.id}')
            print(f'   - current_user.username: {current_user.username}')
            print(f'   - current_user.is_superuser: {getattr(current_user, "is_superuser", None)}')
        ...
        if article:
            ...
            print(f'   - article.hidden: {article.hidden}')
            print(f'   - article.user: {article.user}')
        ...
        # 检查文章是否被删除（status为-1）
        if article.status == -1:
            print(f'   ❌ Article is deleted (status=-1)')
            return ApiResponse(success=False, error="Article not found", data=None)

        # 检查文章是否为隐藏文章（hidden为True）
        # 隐藏文章只对作者、管理员或输入正确密码的用户可见
        if article.hidden:
            print(f'   ⚠️  Article is hidden, checking permissions...')
            
            # 检查用户是否为作者或管理员
            is_author = current_user and current_user.id == article.user
            is_admin = current_user and hasattr(current_user, 'is_superuser') and current_user.is_superuser
            
            print(f'   - is_author: {is_author}')
            ...
            if not is_author and not is_admin:
                ...
                if has_password:
                    # 有密码保护，检查是否已验证
                    access_token = request.cookies.get(f'article_access_{article.id}') or request.query_params.get('access_token')
                    
                    if access_token:
                        # 验证token
                        expected_token = password_protection_service.generate_access_token(article.id)
                        if access_token != expected_token:
                            print(f'   ❌ Invalid access token for password-protected article')
                            return ApiResponse(
                                success=False, 
                        ...
                        # 未提供token，需要密码
                        print(f'   🔒 Password required for this article')
                        return ApiResponse(
                            success=False, 
                            error="Password required",
                            data={
                    ...
                    # 没有密码保护的隐藏文章，拒绝访问
                    print(f'   ❌ Access denied for hidden article without password')
                    return ApiResponse(success=False, error="Article not found", data=None)

        # 检查文章是否为草稿（status为0）
        # 草稿文章只对作者和管理员可见
        if article.status == 0:
            print(f'   ⚠️  Article is draft, checking permissions...')
            # 如果没有登录或不是文章作者或管理员，则禁止访问
            if not current_user or (current_user.id != article.user):
                # 检查用户是否为管理员
                is_admin = current_user and hasattr(current_user, 'is_superuser') and current_user.is_superuser
                print(f'   - is_author: {current_user and current_user.id == article.user}')
                print(f'   - is_admin: {is_admin}')

                if not is_admin:
                    print(f'   ❌ Access denied for draft article')
                    return ApiResponse(success=False, error="Article not found", data=None)
        ...
        if content:
            print(f'   ✅ Article content found (length: {len(content.content) if content.content else 0})')
        else:
            print(f'   ⚠️  Article content not found')
        ...
        # 增加文章浏览量（使用 UPDATE 语句，避免会话状态管理问题）
        ...
        article_data = {
            ...
            "seo_title": seo_data_dict.get('seo_title') if seo_data_dict else None,
            "seo_description": seo_data_dict.get('seo_description') if seo_data_dict else None,
            "seo_keywords": seo_data_dict.get('seo_keywords') if seo_data_dict else None,
            "og_title": seo_data_dict.get('og_title') if seo_data_dict else None,
            "og_description": seo_data_dict.get('og_description') if seo_data_dict else None,
            "og_image": seo_data_dict.get('og_image') if seo_data_dict else None,
            "twitter_title": seo_data_dict.get('twitter_title') if seo_data_dict else None,
            "twitter_description": seo_data_dict.get('twitter_description') if seo_data_dict else None,
            "twitter_image": seo_data_dict.get('twitter_image') if seo_data_dict else None,
            "canonical_url": seo_data_dict.get('canonical_url') if seo_data_dict else None,
...
async def get_articles_by_tag_api(
    ...
    try:
        ...
        articles_query = select(Article).where(
            ...
            Article.is_vip_only == False  # 只返回非 VIP 专属文章
        )

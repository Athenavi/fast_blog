...
class CreateCommentRequest(BaseModel):
    ...
    # 访客信息（未登录用户必填�?    author_name: Optional[str] = None
...
async def create_comment(
    ...
    # 1. 检查文章是否存�?    from shared.models.article import Article
    ...
    spam_check = spam_filter.check_spam(
        ...
        # 5. 敏感词过滤检�?    from shared.services.security.sensitive_word_service import sensitive_word_service
    ...
    if sensitive_check['has_sensitive'] and 'warn' in sensitive_check['actions']:
        is_approved = False  # 强制待审�?    else:
        # 根据垃圾检测结果决定审核状�?        is_approved = spam_check['action'] == 'approve'

    # 6. 如果内容需要替换敏感词，进行替�?    filtered_content = comment_data.content
    if sensitive_check['has_sensitive'] and 'replace' in sensitive_check['actions']:
        ...
        # 服务器端 HTML 转义（防�?XSS，与前端 DOMPurify 双层保护�?    filtered_content = html.escape(filtered_content)

        # 5. 根据检测结果决定操�?    if spam_check['action'] == 'reject':
    ...
    if author_url:
        ...
        if parsed.scheme not in ('http', 'https'):
            author_url = None
            # 拒绝 javascript: �?data: 等危险协�?        elif parsed.scheme in ('javascript', 'data', 'vbscript'):
            author_url = None
    ...
    new_comment = Comment(
        ...
        user_agent=user_agent,
        # 垃圾检测信�?        spam_score=spam_check['confidence'],
    ...
    if is_approved:
        try:
            ...
            # 触发关注插件的评论事�?            try:
                ...
                # 获取文章作者信�?            if article and article.user:
                ...
                if article_author and article_author.email:
                    # 获取评论者信�?                    commenter_name = current_user.username if current_user else comment_data.author_name or '访客'
                    ...
                    # 通知文章作�?                    comment_notification_service.notify_article_author(
                ...
                # 如果有父评论，通知被回复�?            if new_comment.parent_id:
                ...
                # 通知被回复�?                    comment_notification_service.notify_comment_reply(
...
async def get_article_comments(
    ...
    Args:
        ...
        order: 排序方向 (asc-升序, desc-降序) - 仅用于向后兼�?        tree: 是否返回树形结构（嵌套回复）
    """
    ...
    # 如果要求树形结构: 先分页获取根评论,再递归获取子评�?    if tree:
        # 1. 获取根评论总数
    ...
    # 3. 一次性查询所有后代评论（避免 N+1�?        all_comment_ids = set(c.id for c in root_comments)
        ...
        while parent_ids:
            ...
            # 下一层继续批量查�?            parent_ids = [c.id for c in children]
    ...
    # 6. 构建�?        tree_data = _build_comment_tree(comments_data)
        result_data = {
            'comments': tree_data,
            'total': total,
            'page': page,
...
# 获取所有用户信�?        user_ids = [c.user_id for c in comments if c.user_id]
        users = {}
        if user_ids:
            user_query = select(User).where(User.id.in_(user_ids))
            user_result = await db.execute(user_query)
...
async def update_comment(
    ...
    if not comment:
        ...
        # 检查权�?- 只有评论作者或管理员可以编�?    if comment.user_id != current_user.id and not getattr(current_user, 'is_staff', False):
...
async def delete_comment(
    ...
    if not comment:
        ...
        # 检查权�?- 只有评论作者或管理员可以删�?    if comment.user_id != current_user.id and not getattr(current_user, 'is_staff', False):
...
async def like_comment(
    ...
    if not comment:
        ...
        # 检查是否已经投�?    stmt = select(CommentVote).where(
    ...
    if existing_vote:
        if existing_vote.vote_type == 1:
            # 已经点赞，取消点�?            await db.delete(existing_vote)
...
async def dislike_comment(
    ...
    if existing_vote:
        if existing_vote.vote_type == -1:
            # 已经反对，取消反�?            await db.delete(existing_vote)
            action = 'undisliked'
            message = '已取消反�?
        else:
            # 之前是点赞，改为反对
...
async def notify_comment_reply(
    ...
    # 获取父评�?    stmt = select(Comment).where(Comment.id == new_comment.parent_id)
    result = await db.execute(stmt)
...
def _build_comment_tree(comments: list) -> list:
    ...
    for comment_dict in comment_map.values():
        ...
        # 根评�?            root_comments.append(comment_dict)
        else:
            # 子评论，添加到父评论�?children
...
async def list_pending_comments(
    ...
    # 附上文章标题和用户信�?    from shared.models.article import Article
    from shared.models.user import User

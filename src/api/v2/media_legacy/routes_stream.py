...
router = APIRouter(tags=["media-file"])
...
async def get_media_file_by_id(
    ...
    try:
        ...
        if not file_stat and file_hash.storage_path and file_hash.storage_path.startswith("local://"):
    ...
    # 如果标准路径不存在，尝试用 storage_path 构建完整路径
    if file_hash.storage_path:
        ...
        if not file_hash.storage_path.startswith(("s3://",)):
            ...
            try:
                ...
                raise HTTPException(status_code=403, detail="非法的文件路径")
            logger.info("  尝试用 storage_path 构建路径")
            if full_path.exists():
    ...
    if file_hash.storage_path and file_hash.storage_path.startswith("s3://"):
        ...
        logger.error("  [ERROR] 不支持的存储类型")
        logger.error("  文件不存在于标准路径和 storage_path")

...
from fastapi import APIRouter, Depends, Body, Query, Request, UploadFile, File
...
async def update_media(
    ...
    return ok(data={"message": "更新成功", "media": media.to_dict()})
...
async def upload_edited_image(
    ...
    try:
        ...
        await db.commit()
        return ok(data=media.to_dict(), msg="图片保存成功")
    except Exception:
        await db.rollback()
        raise

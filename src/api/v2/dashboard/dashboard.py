...
async def get_blog_management_articles(
    ...
    """
    # 使用SQLAlchemy异步查询语法
    # 构建基础查询（category 是外键字段，不是 relationship，不能直接使�?selectinload...
    ...
    if status:
        ...
        if status_lower in status_map:
            if status_lower == 'deleted':
                # deleted �?status=-1 �?deleted_at 标记（删除时两者同时写入）...
                    # 过滤与展示逻辑保持一致，兼容历史仅设 deleted_at 的数...
    ...
    for article in articles:
        ...
        # 处理标签（tags_list 已经�?JSON 数组...
            tags_list = article_dict.get('tags_list') or []

        # 构建响应数据，在 to_dict() 基础上添加关联数...
            articles_data.append({
            **article_dict,  # 展开模型的基础字段
            "summary": article_dict.get('excerpt', ''),  # summary �?excerpt 的别...
                "tags": tags_list,  # 转换后的标签数组
            "views_count": article_dict.get('views', 0),  # 前端期望的字段名
            "status": article_status,  # 覆盖为字符串状...
                "author": author_info,  # 添加作者信...

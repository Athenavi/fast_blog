...
async def create_revision_note(
    ...
    return ok(data=note.to_dict(), msg="修订注释创建成功")
...
async def update_revision_note(
    ...
    return ok(data=note.to_dict(), msg="修订注释更新成功")
...
async def list_menu_locations(
    ...
    return ok(data={"menu_locations": [loc.to_dict() for loc in locations]})
...
async def create_menu_location(
    ...
    return ok(data=location.to_dict(), msg="菜单位置创建成功")
...
async def update_menu_location(
    ...
    if "slug" in data and data["slug"] != location.slug:
        ...
        if existing.scalar_one_or_none():
            return fail(f"标识 '{data['slug']}' 已被其他位置使用")
    ...
    return ok(data=location.to_dict(), msg="菜单位置更新成功")
...
async def create_assignment(
    ...
    if not menu_id or not location_id:
        return fail("menu_id 和 location_id 为必填字段")
    ...
    if not menu_result.scalar_one_or_none():
        return fail(f"菜单 ID={menu_id} 不存在")
    ...
    if not loc_result.scalar_one_or_none():
        return fail(f"菜单位置 ID={location_id} 不存在")
    ...
    return ok(data=assignment.to_dict(), msg="菜单-位置关联创建成功")

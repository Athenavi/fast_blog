...
async def get_my_badges(
    ...
    """获取当前用户已获得的徽章"""
    badges = achievement_badge_system.get_user_badges(current_user.id, category=category)
    return ok(data={'badges': badges, 'count': len(badges)})


@router.get("/available", summary="获取所有可用徽章")
@_catch
async def get_available_badges(category: str = Query(None, description="分类过滤")):
    """获取系统中所有可用的徽章"""
...
async def get_badge_progress(
    ...
    """获取指定徽章的完成进度"""
    progress = achievement_badge_system.get_badge_progress(current_user.id, badge_key)
...
async def admin_award_badge(
    ...
    if success:
        ...
        return ok(data={'user_id': user_id, 'badge_key': badge_key, 'badge_name': badge_details['name']},
                  message=f'成功为用户 {user_id} 授予徽章: {badge_details["name"]}')
    return fail('授予失败(徽章不存在或已获得)')


@router.get("/admin/stats", summary="获取徽章系统统计")
@_catch
async def get_badge_system_stats(current_user: UserModel = Depends(admin_required_api)):
    """获取徽章系统整体统计"""
    total_badges = len(achievement_badge_system._badges)
    categories = achievement_badge_system.get_categories()
    total_awarded = sum(len(badges) for badges in achievement_badge_system._user_badges.values())
    return ok(data={'total_badges': total_badges, 'categories': categories, 'total_awarded': total_awarded})

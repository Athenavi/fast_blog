"""
TOTP 双因素认证服�?支持 TOTP 验证�?FA 启用/禁用、备用码管理
"""
import hashlib
import hmac
import struct
...
class TwoFactorService:
    ...
    # 已使用的 TOTP 时间�?(user_id -> set of time_step), 防止重放攻击
    ...
    async def setup_totp(
        ...
        user.is_2fa_enabled = False  # 待验证后才启�?        await db.commit()

        # 生成备用�?        backup_codes = self._generate_backup_codes()
        ...
        return {
            ...
            "message": "请使�?Authenticator 应用扫描二维码，然后调用 verify_totp 完成验证",
        }
    ...
    async def verify_and_enable(
        ...
        if not user.totp_secret:
            return {"success": False, "error": "请先调用 setup_totp 初始�?}

        if user.is_2fa_enabled:
            return {"success": False, "error": "双因素认证已启用"}

        # 验证 TOTP �?        totp = pyotp.TOTP(user.totp_secret)
        if not totp.verify(code, valid_window=1):
            return {"success": False, "error": "验证码无效，请重�?}
        ...
        for step in (current_step - 1, current_step, current_step + 1):
            if totp.verify(code, valid_window=0, time=step * 30):
                if step in used:
                    return {"success": False, "error": "该验证码已被使用过，请等待新验证�?}
                used.add(step)
                break
    ...
    async def verify(
        ...
        if not user.is_2fa_enabled:
            return {"success": True, "message": "2FA 未启�?}

        # 尝试 TOTP 验证
        if user.totp_secret:
            totp = pyotp.TOTP(user.totp_secret)
            if totp.verify(code, valid_window=1):
                # 防止 TOTP 重放攻击: 记录已使用的时间�?                current_step = self._get_totp_time_step()
                self._cleanup_used_steps(user_id)
                used = self._used_totp_steps.setdefault(user_id, set())
                # 检查该时间步是否已被使用过
                for step in (current_step - 1, current_step, current_step + 1):
        ...
        # 尝试备用�?        if user.backup_codes:
            ...
            if len(backup_list) == 1:
                ...
                try:
                    ...
                    pass  # 不是 JSON 格式，回退�?plaintext 处理
            ...
            for i, backup_code in enumerate(backup_list):
                ...
                if self._constant_time_compare(code.strip(), backup_code):
                    # 移除已使用的备用�?                    remaining = backup_list[:i] + backup_list[i + 1:]
                    user.backup_codes = "\n".join(remaining) if remaining else None
                    await db.commit()
                    return {"success": True, "method": "backup_code"}

        return {"success": False, "error": "验证码无�?}
    ...
    async def regenerate_backup_codes(
        ...
        if not user.is_2fa_enabled:
            return {"success": False, "error": "2FA 未启�?}
